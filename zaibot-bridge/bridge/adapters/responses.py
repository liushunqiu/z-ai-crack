...
def build_cached_response(
    response_id: str, events: list[InternalStreamEvent], model: str
) -> dict:
    """聚合事件成 GET /v1/responses/{id} 可返回的完整 response。"""
    full_text: list[str] = []
    ...
    for e in events:
        if isinstance(e, TextDelta):
            full_text.append(e.chunk)
        elif isinstance(e, ThinkingDelta):
            full_reasoning.append(e.chunk)
        elif isinstance(e, ToolCallStart):
            tool_outputs[e.call_id] = {
                "id": e.call_id,
                "call_id": e.call_id,
                "type": "function_call",
                "name": e.name,
            ...
            tc = tool_outputs.get(e.call_id)
            if tc:
                tc["arguments"] = (tc.get("arguments") or "") + e.arguments_delta
    ...
    if full_reasoning:
        output.append({
            ...
            "id": "reasoning_0",
            "status": "completed",
            "summary": [{"type": "summary_text", "text": "".join(full_reasoning)}],
        })
    if full_text:
        output.append({
            "type": "message",
